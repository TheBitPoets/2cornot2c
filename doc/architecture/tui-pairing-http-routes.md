...
Un unico store SQLite atomico, condiviso col login Google, applica bucket globali e per-client distinti a begin, authorize, consume e logout. Consume aggiunge un bucket HMAC per pairing, impedendo tentativi distribuiti eccessivi sullo stesso identificatore. IP, pairing e codici non vengono persistiti nei bucket in chiaro.
