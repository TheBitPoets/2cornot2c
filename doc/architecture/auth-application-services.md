...
Una sessione revocata o appartenente a un account disabilitato fallisce chiusa. Creazione sessione e verifica `active` avvengono nella stessa transazione e richiedono la revisione `users.updated_at` letta prima di generare il bearer, così un ciclo disabilitazione/riabilitazione concorrente non può convalidarlo. Ogni autenticazione, anche senza avanzamento di `last_seen_at`, esegue un CAS atomico su identità/generazione completa della sessione, account attivo e revisione `users.updated_at`; un cambio ruolo concorrente viene riletto prima di restituire l'account, mentre una sostituzione della generazione non viene adottata dal tentativo stale. Dopo `BEGIN IMMEDIATE`, lo storage ricampiona il proprio clock e richiede `expires_at > max(expected_valid_at, proposed_last_seen_at, storage_time)` nello stesso UPDATE che effettua il touch. Una sessione che scade durante l'attesa del lock fallisce quindi come credenziale non valida. Il touch persiste e restituisce lo stesso massimo conservativo come nuovo `last_seen_at`, creando un high-water mark durevole che impedisce anche a un rollback successivo del clock storage di arretrare l'osservazione di validità. Una race con revoca viene riletta e non puo riattivare il bearer. Una modifica concorrente ancora attiva durante la revoca produce un errore esplicito, non un falso successo. Un rollback dell'orologio sotto `last_seen_at` viene rifiutato.
