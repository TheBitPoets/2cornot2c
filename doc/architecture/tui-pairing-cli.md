...
8. conserva insieme al bearer un proof logout HMAC domain-separated, redatto e memory-only;
9. in un blocco `finally`, anche dopo errore o interruzione terminale, invia `POST /auth/tui/logout` con bearer e proof e poi elimina i riferimenti locali. Se la conferma remota manca, l'uscita fallisce con un messaggio sanificato.

Il proof permette al server di scartare e limitare bearer inventati senza interrogare SQLite, mentre il logout legittimo non può essere bloccato dai bucket anonimi. Entro la deadline assoluta il client ritenta 429 secondo `Retry-After` e 503 transitori con backoff bounded. Il logout accetta soltanto un 204 con body vuoto, senza Content-Type, Content-Encoding o Transfer-Encoding e con Content-Length assente o zero. Redirect, 200 con JSON e risposte riflesse falliscono chiusi. La revoca server precede la risposta: la perdita della risposta può impedire la conferma client, ma non riattiva la sessione.
