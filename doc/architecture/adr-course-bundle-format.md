...
- Tutti i path finali prodotti (file locali, importati e override) devono essere univoci dopo normalizzazione Unicode NFC e confronto case-folded; qualsiasi collisione tra origini diverse fallisce senza sovrascrittura. I path in `content.units` e `index.json` sono riferimenti e possono legittimamente puntare a un `target_path` importato.
