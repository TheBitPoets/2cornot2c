...
L'activity originale resta autorevole per consegna, rubriche e correzione; il link conserva soltanto i metadati necessari alla pianificazione. I bundle asset sono pubblicati in directory immutabili identificate dal digest e il JSON activity viene sostituito atomicamente: un arresto durante l'overwrite non modifica il bundle ancora referenziato. Un bundle nuovo dall'esito incerto viene conservato come orphan innocuo invece di rischiare di cancellare asset gia referenziati; la raccolta degli orphan e un'operazione di manutenzione separata. I percorsi asset legacy `assets/<slug>/<file>` restano leggibili e preservabili per compatibilita con le bozze preesistenti, mentre ogni nuova pubblicazione usa esclusivamente bundle immutabili content-addressed. La cancellazione di una bozza e serializzata con i salvataggi dei percorsi e viene bloccata finche un design corrente o archiviato la referenzia. Dopo la cancellazione non e possibile creare un nuovo link pendente perche il salvataggio richiede nuovamente il file autorevole.

La Course Board mostra le activity disponibili in ogni UDA e usa un dialog esplicito per aggiungere, modificare o rimuovere un collegamento, scegliere il ruolo e impostare pianificazione e scadenza. Una lista vuota inviata dal wizard activity non altera implicitamente i collegamenti gia approvati.

Il calendario richiede `GET /api/course-calendar-context` per ottenere, dallo stesso snapshot, il design selezionato, la sua revisione e gli `activity_events` derivati da `iter_scheduled_activity_links()`. Gli eventi vengono filtrati insieme ai percorsi visibili e resi come activity pianificata o scadenza; non vengono mai copiati nel JSON del calendario.

Design e calendari usano revisioni content-addressed e compare-and-swap: una sessione obsoleta riceve HTTP 409 invece di sovrascrivere modifiche concorrenti. Il calendario aggiorna la programmazione svolta attraverso `POST /api/course-uda-actual`, che fonde solo `uda.actual` nel design piu recente e conserva activity link e modifiche della Course Board. I salvataggi completi dello stesso design preservano il consuntivo piu recente; le sostituzioni esplicite tra progetti diversi non lo ereditano.
