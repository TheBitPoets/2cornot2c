...
8. `logrotate --debug /etc/logrotate.conf` valida simultaneamente policy distro e pilot senza entry duplicate; rotazione+`systemctl kill --kill-whom=main --signal=USR1 nginx.service` ricrea file `0640` e una scrittura successiva entra soltanto nel nuovo path;
...
`nginx/thebitlab-process-error-log.conf` entra invece nel contesto `main` e conserva in `origin.error_log`, a livello `notice`, diagnostica di ciclo/master/worker non associata alle richieste. La topologia Ubuntu impone, non rende configurabili, `logging.directory=/var/log/thebitlab`, directory `root:www-data` `0750` e file access/process `www-data:adm` `0640`, figli diretti distinti `.log`. L'attivatore rifiuta ACL POSIX nominate, mask estese e default ACL tramite `getfacl`; owner/mode errati senza ACL vengono riparati e poi verificati. Il validator rifiuta `/var/log/nginx` e quindi evita la wildcard del package. Logrotate applica `daily`, `rotate 30`, `maxage 30`, compressione differita, ricrea `0640`, vieta `copytruncate` e interroga `systemctl is-active nginx.service`: `active` richiede `systemctl kill` sul master identificato dalla unit, attende bounded l'elaborazione asincrona di USR1 e riconferma la unit active; ogni failure fallisce la rotazione. Soltanto `inactive` consente di non segnalare. `failed`, `activating`, `deactivating`, unknown o manager indisponibile falliscono chiusi. Per la segnalazione logrotate, PID file e lookup `/proc` ad hoc non sostituiscono mai l'identità della unit systemd. Incident/legal hold seguono un'eccezione separata e motivata, non una retention implicita.
