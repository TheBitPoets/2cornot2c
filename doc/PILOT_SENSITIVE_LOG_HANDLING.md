...
- Gli esiti delle richieste restano auditabili nell'access log. Il log errori nginx è limitato a `crit`, perché i livelli request-context possono riportare il target non redatto; le diagnostiche applicative restano soggette ai test secret-safe.
- `logging.rotation=daily`, `retention_days=30` e `file_mode=0640` sono vincoli del manifest. La configurazione logrotate usa compressione, riapertura del file senza `copytruncate` e massimo 30 giorni, coerente con la governance #699.
...
3. Eseguire scanner e inventario metadata-only (path approvato, dimensione, timestamp, digest), conservando soltanto esito/conteggi sanitizzati. Lo scanner non stampa il contenuto:
   ...
   python scripts/pilot_access_log_scanner.py \
     /var/log/nginx/thebitlab-access.log \
     /var/log/nginx/thebitlab-error.log
   ```
...
5. Preparare sullo stesso filesystem una directory di quarantena cifrata/root-only (`0700`), con nome non sensibile e inventario esatto. Spostare atomicamente **solo i file approvati**, senza wildcard, preservando metadata; precreare i nuovi path con owner/gruppo del manifest e modo `0640`, quindi inviare `USR1` al master nginx per la riapertura. Verificare subito che nessun processo continui a scrivere nello storico. Se una di queste precondizioni non è verificabile, fermarsi ed escalare: non usare `copytruncate`.
6. Attivare il bundle esatto e il link versionato logrotate; eseguire `nginx -t`, `logrotate --debug`, reload controllato e smoke callback/ordinario. Verificare owner, gruppo e `0640` sui nuovi file, path-only, status/timing/request ID e scanner verde senza pubblicare il log.
