...
6. Attivare il bundle esatto e i link versionati main-context nginx, formato/site, logrotate e systemd; eseguire `nginx -t`, `logrotate --debug`, reload controllato e smoke callback/ordinario/error-path/default-host. Verificare owner, gruppo e `0640` sui nuovi file, path-only, status/timing/request ID, diagnostica process-level presente e scanner verde senza pubblicare il log.
