...
6. Dopo il bootstrap amministrativo separato della toolchain pinnata in `/usr/lib/thebitlab/pilot-tools/<id>` e del launcher `/usr/sbin/thebitlab-pilot-activate`, copiare la candidate v2 in `/etc/thebitlab/deployments` con albero root-owned non scrivibile da group/other. Non eseguire mai l'attivatore dal checkout. Il preflight accetta soltanto il fingerprint legacy esatto; poi l'attivatore usa `systemctl mask --now`, verifica mask manager, inattività e start negativo e solo allora scrive lo state/switcha. AST logging a ogni profondità, renderer della toolchain installata, host trust, `nginx -t/-T`, logrotate globale e systemd devono passare prima dell'unmask. In caso di crash usare soltanto `/usr/sbin/thebitlab-pilot-activate recover`; non rimuovere il guard a mano. Cercare marker nei log dedicati, `/var/log/nginx/*.log`, file ruotati e journal stdout/stderr, anche dopo rollback v2→v2. Availability loss è accettabile; riattivare v1/default o logging query-bearing non lo è.
