...
- La topologia Ubuntu impone `/var/log/thebitlab` `root:adm` `0750` e file `www-data:adm` `0640`, senza ACL nominate/default. Logrotate usa compressione e massimo 30 giorni, senza `copytruncate` né sovrapposizione a `/var/log/nginx/*.log`; la riapertura usa esclusivamente l'identità `nginx.service` (`systemctl kill --kill-whom=main --signal=USR1`), mai PID file o `/proc`, e fallisce su stati diversi da active/inactive non ambiguo.
