...
6. Dopo il bootstrap amministrativo separato della toolchain pinnata in `/usr/lib/thebitlab/pilot-tools/<id>` e del launcher `/usr/sbin/thebitlab-pilot-activate`, copiare la candidate v2 in `/etc/thebitlab/deployments` con albero root-owned non scrivibile da group/other. Non eseguire mai l'attivatore dal checkout. Il preflight accetta soltanto il fingerprint legacy esatto e inventari chiusi: unit/drop-in unmanaged sono rifiutati anche se disabled o se nascondono nginx dietro un wrapper; un output sotto `/run/systemd/generator*` richiede provenance package del proprio input, non soltanto del generator. Config e binary `.so` dei moduli dinamici nginx devono essere entrambi attribuiti a package Ubuntu installati; moduli nativi locali/unmanaged sono vietati sul dedicated host. Analogamente, `logrotate.timer/service` package-owned non rendono trusted configurazioni locali: `/etc/logrotate.conf` e ogni ordinary entry di `/etc/logrotate.d` devono avere exact owner package installato e bytes coincidenti con la provenance dpkg; l’unica entry managed è l’exact symlink TheBitLab verso l’artifact locked current. `logrotate --debug` prova soltanto la sintassi. Poi l'attivatore usa `systemctl mask --now`, verifica mask manager, inattività e start negativo e solo allora scrive lo state/switcha. AST logging a ogni profondità, renderer della toolchain installata, host trust, `nginx -t/-T`, inventario logrotate e boot surface systemd devono passare prima dell'unmask e dell'enable finale. Il gate di boot applica una registry chiusa ai timer root: `apt-daily*`, `e2scrub_all`, `logrotate` e `motd-news` possono restare raggiungibili soltanto dopo il rispettivo input attestor; file APT locali, `/etc/e2scrub.conf` modificato, source motd locali e qualunque nuovo timer root `UNKNOWN` bloccano prima dell’esecuzione. La provenance è basata su owner/stato/digest package e non sul testo del comando. In caso di crash usare soltanto `/usr/sbin/thebitlab-pilot-activate recover`; non rimuovere il guard a mano. Cercare marker nei log dedicati, `/var/log/nginx/*.log`, file ruotati e journal stdout/stderr, anche dopo rollback v2→v2. Availability loss è accettabile; riattivare v1/default o logging query-bearing non lo è.
