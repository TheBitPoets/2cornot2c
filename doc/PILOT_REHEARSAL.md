...
6. soltanto dopo questa verifica aprire la dashboard docente servita dalla stessa istanza e, nel pannello `Genera registro consegne`, selezionare l'assegnazione demo; la selezione propone automaticamente un path basato sulla classe, quindi impostare esplicitamente **Output registro** a `demo/python-demo-somma-001.json`, creare il registro e verificare che proprio `demo/python-demo-somma-001.json` resti selezionato e caricato; nel file `$DemoRoot\teacher-reports\demo\python-demo-somma-001.json` la riga `rossi-mario` deve avere `submission.report_selection=final`, `submission.final_selected=true` e lo stesso `submission.attempt_id` scelto nella TUI, mentre la dashboard deve mostrare lo stesso esito e conteggio test; un output diverso, un registro non rigenerato dopo la selezione o un registro diverso da quello caricato rende il gate `FAIL`.
...
Eseguire tutti gli Scenari 1-10 di [SCENARI_TEST_MANUALI_GUI.md](SCENARI_TEST_MANUALI_GUI.md), nell'ordine TUI → studente → docente: gli Scenari 7A, 6 e 7 già completati nella sezione 3, quindi 1 e 8, poi 2-5 e infine 9-10. Per l'intero rehearsal sostituire le root `tmp/...` del documento generico con `$DemoRoot` e avviare sempre il server con `--root $DemoRoot`. Nello Scenario 1 saltare il comando `student_lab_demo_setup.py` delle precondizioni: usare il server già attivo sulla `$DemoRoot` oppure riavviare soltanto `course_board_server.py --root $DemoRoot`, senza ricreare la demo. Non ripetere gli scenari TUI già completati e, dopo il caso positivo dello Scenario 8, non eseguire il setup indicato dal documento generico per tornare al caso negativo: cancellerebbe tentativi, definitivo e aiuti che devono essere verificati dagli scenari docente. Una funzione deliberatamente esclusa va marcata `N/A` con decisione e impatto; i flussi usati nella prima lezione non possono essere esclusi.

Per gli Scenari 2-4 il registro è intenzionalmente quello rigenerato alla fine della sezione 3, non la fixture appena creata dal setup. Applicare quindi queste aspettative sostitutive:
