...
All'avvio il runtime firma un JWT RS256 retrodatato in modo conservativo, richiede un installation token soltanto a `https://api.github.com`, ne valida scadenza e formato e lo pubblica atomicamente nel `token_file`. Windows usa `ReplaceFileW`; Linux usa `renameat2(RENAME_EXCHANGE)` e macOS `renameatx_np(RENAME_SWAP)`. Un filesystem POSIX che non offre lo scambio atomico viene rifiutato invece di introdurre finestre senza credenziale. Il rinnovo avviene prima della scadenza; errori successivi non cancellano una credenziale ancora valida. Allo shutdown viene eliminata soltanto l'esatta generazione ancora posseduta dal processo, senza rimuovere un token sostituito da un altro runtime. Un lock nella directory protetta impedisce due processi concorrenti sulla stessa installazione.
